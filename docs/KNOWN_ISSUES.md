...
- classic 28-byte 후보 구조를 지원한다.
- `_EXTB$` 확장 IDX는 16-byte header + 128-byte entry 후보 구조를 지원한다.
- ExtB compressed size는 offset 정렬과 다음 offset 차이를 기준으로 계산한다.
- ExtB 마지막 entry의 compressed size는 PAK 파일 크기를 기준으로 계산한다.
- ExtB compression 1은 zlib, compression 2는 brotli 후보로 처리한다.
- ExtB 압축 해제는 실제 샘플 IDX/PAK 검증 전 단계이므로 클라이언트 변형에 따라 실패할 수 있다.
- 보호/암호화 IDX는 아직 지원하지 않는다.
- IDX 포맷이 다른 클라이언트에서는 목록/offset/size/compression 정보가 정확하지 않을 수 있다.
