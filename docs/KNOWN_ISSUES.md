...
- TIL/IMG/SPR/TBT는 변환기 pipeline과 진단 기능만 준비되어 있다.
- TIL/IMG/SPR 실제 이미지 변환/렌더링은 아직 구현되지 않았다.
...
- Tile.idx/Tile.pak는 상태/검색/진단 연결까지만 되어 있고 실제 타일 이미지 캐시는 일반 이미지 포맷에 한정된다.
